...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/admin/users", requireAdmin, async (req, res) => {
    try {
      const { username, password, firstName, lastName, email, dateOfBirth, socialSecurityNumber, address, primaryPhone, secondaryPhone } = req.body;
  ...
  app.patch("/api/admin/users/:userId", requireAdmin, async (req, res) => {
    try {
      const userId = parseInt(req.params.userId);
      const { 
        firstName, lastName, email, dateOfBirth, socialSecurityNumber, 
        occupation, homeAddress, mailingAddress, primaryPhone, 
        secondaryPhone, alternateEmail 
      } = req.body;
      ...
      if (req.body.transactions !== undefined) updateData.transactions = req.body.transactions;
      if (req.body.availableBalance !== undefined) updateData.availableBalance = req.body.availableBalance;
      if (req.body.everydayChecking !== undefined) updateData.everydayChecking = req.body.everydayChecking;
